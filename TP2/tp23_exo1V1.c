...
void P(pthread_mutex_t *mutex)
{
  ...
  {
    thdErreur(1, "P: error locking mutex", NULL);
  }
}
...
void V(pthread_mutex_t *mutex)
{
  ...
  {
    thdErreur(1, "V: error unlocking mutex", NULL);
  }
}
...
int main(int argc, char *argv[])
{
  ...
  mutex = malloc(nbThreads * sizeof(pthread_mutex_t));

  for (int i = 0; i < nbThreads; i++)
  {
    if (pthread_mutex_init(&mutex[i], NULL) != 0)
      thdErreur(1, "Error initializing mutex", NULL);
  }
  ...
  for (int i = 0; i < nbThreads; i++)
  {
    if (pthread_mutex_destroy(&mutex[0]) != 0)
      thdErreur(1, "Error destroying mutex", NULL);
  }
    free(mutex);
